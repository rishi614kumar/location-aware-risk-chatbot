...
def get_surrounding_features_from_bbl(
    bbl: str,
    *,
    mode: Literal["radius", "street"],
    source_gdf: Optional[gpd.GeoDataFrame] = None,  # ✅ 新增
    radius_ft: Optional[Union[int, float]] = None,
    street_buffer_ft: Optional[Union[int, float]] = None,
    id_field: str = "BBL",  # ✅ 可以指定唯一标识字段
    include_self: bool = False,
) -> List[str]:
    """
    Given a BBL, find surrounding features (BBLs or others) in a given dataset.
    The dataset may be PLUTO or any GeoDataFrame with geometry.
    """
    # load data
    ...
    # Use provided dataset or default to PLUTO
    if source_gdf is None:
        source_gdf = pluto
        print("🔹 Using PLUTO as source data.")
    else:
        print(f"🔹 Using provided dataset with {len(source_gdf)} records.")

    # radius mode
    if mode == "radius":
        if radius_ft is None:
            raise ValueError("mode='radius' requires radius_ft.")
        scope_geom = target.buffer(float(radius_ft)).unary_union
        scope_gdf = gpd.GeoDataFrame(geometry=[scope_geom], crs=pluto.crs)
        joined = gpd.sjoin(source_gdf, scope_gdf, how="inner", predicate="intersects")
    ...
    # street mode
        ...
        hit_segments = gpd.sjoin(lion_buf, target[["geometry"]], how="inner", predicate="intersects")
        if hit_segments.empty:
            print(f"⚠️ No street segments intersect BBL {bbl}")
            return []
        scope_geom = unary_union(hit_segments.geometry.values)
        scope_gdf = gpd.GeoDataFrame(geometry=[scope_geom], crs=pluto.crs)
        joined = gpd.sjoin(source_gdf, scope_gdf, how="inner", predicate="intersects")
    else:
        raise ValueError("mode must be 'radius' or 'street'.")

    # Extract unique IDs
    if id_field in joined.columns:
        ids = joined[id_field].astype(str).unique().tolist()
    else:
        ids = [f"feature_{i}" for i in range(len(joined))]

    # Include/exclude self
    if not include_self and id_field == "BBL":
        ids = [i for i in ids if i != str(bbl)]
