...
(function() {
  ...
  function buildLanding() {
    ...
    // Decorative crown glyph background
    ...
    tagline.textContent = 'No more digging through spreadsheets or maps—this system converts complex geospatial data into simple, conversational answers tailored to each project site.';
    ...
    features.forEach(f => {
      ...
      li.innerHTML = `<span style="display:inline-flex;align-items:center;gap:.55rem"><span style="background:var(--cu-blue);color:var(--cu-navy);width:22px;height:22px;display:inline-flex;align-items:center;justify-content:center;border-radius:999px;font-size:.75rem;font-weight:700;box-shadow:0 4px 10px rgba(0,51,102,.25)">✓</span>${f}</span>`;
      ul.appendChild(li);
    });
    ...
    startBtn.setAttribute('aria-label', 'Enter Chat and begin conversation');
    ...
    function makeLink(text, href) {
      ...
      a.addEventListener('mouseenter', () => { a.style.background = 'color-mix(in oklab, var(--cu-blue-50) 55%, transparent)'; });
      a.addEventListener('mouseleave', () => { a.style.background = 'var(--surface)'; });
      return a;
    }

    links.appendChild(makeLink('Repository', 'https://github.com/rishi614kumar/location-aware-risk-chatbot'));
    links.appendChild(makeLink('NYC Open Data', 'https://opendata.cityofnewyork.us'));

    // Footer / disclaimer
    const disclaimer = document.createElement('small');
    disclaimer.textContent = 'Made in collaboration with Town+Gown: NYC @ DDC. LLM output may contain inaccuracies.';
    ...
    // Accessibility: trap focus within landing until dismissed
    ...
    root.addEventListener('keydown', e => {
      if (e.key === 'Tab') {
        ...
        if (e.shiftKey) {
          if (idx <= 0) { items[items.length - 1].focus(); e.preventDefault(); }
        } else {
          if (idx === items.length - 1) { items[0].focus(); e.preventDefault(); }
        }
      }
    });
  ...
  // Inject a tiny keyframe for fadeIn if not present
  function ensureKeyframe() {
    if (document.getElementById('cu-landing-anim')) return;
    const style = document.createElement('style');
    style.id = 'cu-landing-anim';
    style.textContent = '@keyframes fadeIn { from { opacity:0; transform:translateY(8px) scale(.985);} to { opacity:1; transform:translateY(0) scale(1);} }';
  ...
  function hideLanding(storeDismissal = true) {
    ...
    const composer = document.querySelector('.cl-composer textarea, .cl-composer input, textarea.cl-input');
  ...
  function ensureReturnButton() {
    ...
    btn.setAttribute('aria-label', 'Show the introductory landing overlay again');
  ...
  function setupObservers() {
    if (observer) return;
    observer = new MutationObserver(() => ensureReturnButton());
  ...
  function init() {
    ensureKeyframe();
    if (!sessionStorage.getItem(STORAGE_KEY)) showLanding();
    else ensureReturnButton();
    setupObservers();
  }
